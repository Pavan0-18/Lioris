...
export default function EditProductPage() {
  ...
  const { data: lookupsData, isLoading: lookupsLoading } = useQuery({
  ...
  const { data: productData, isLoading: productLoading } = useQuery({
    ...
    queryFn: () => fetch(`/api/tenant/inventory/products/${productId}`).then((r) => r.json()),
    staleTime: 5 * 60 * 1000,
  });

  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await fetch(`/api/tenant/inventory/products/${productId}`, {
        method: "PATCH",
  ...
  if (lookupsLoading || productLoading) {
    return <BoneyardPage />;
  }
  ...
  if (!product) {
    return (
      <div className="text-center py-16">
        ...
        <Button variant="outline" size="sm" className="mt-4" onClick={() => router.push("/inventory/products")}>
          Back to Products
        </Button>
      </div>
    );
